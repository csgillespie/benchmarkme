run_benchmarks = function(bm, runs, verbose, cores) {
  if (cores > 0) {
    results = lapply(bm, bm_parallel, runs = runs, verbose = verbose, cores = cores)
  } else {
    results = lapply(
      bm,
      do.call,
      list(runs = runs, verbose = verbose),
...
benchmark_prog = function(runs = 3, verbose = TRUE, cores = 0L) {
  bm = c("bm_prog_fib", "bm_prog_gcd", "bm_prog_hilbert", "bm_prog_toeplitz", "bm_prog_escoufier")
  if (verbose) message("# Programming benchmarks (5 tests):")
...
benchmark_matrix_cal = function(runs = 3, verbose = TRUE, cores = 0L) {
  ...
  if (verbose) message("# Matrix calculation benchmarks (5 tests):")
...
benchmark_matrix_fun = function(runs = 3, verbose = TRUE, cores = 0L) {
  ...
  if (verbose) message("# Matrix function benchmarks (5 tests):")
  run_benchmarks(bm, runs, verbose, cores)
}
